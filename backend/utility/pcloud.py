...
def uploadFile(auth, data, folderPath=None, folderid=None):
    ...
    if folderid:
        response = requests.post(
            f'{BASE_URL}/uploadfile?auth={auth}&folderid={folderid}&nopartial=1&renameifexists=1&filename=medico', files=file)
    elif folderPath:
        response = requests.post(
            f'{BASE_URL}/uploadfile?auth={auth}&path={folderPath}&nopartial=1&renameifexists=1&filename=medico', files=file)
    response = response.json()
    if response['result'] == 0:
...
def getPubSmallThumb(code, size, png=0):
    return f'{BASE_URL}/getpubthumb?code={code}&size={f"{size}x{size}"}&type={"png" if png==1 else "jpg"}'


# return all file id's in the given folder Id as list
def getItemsInFolder(auth, folderid):
    response = requests.post(
